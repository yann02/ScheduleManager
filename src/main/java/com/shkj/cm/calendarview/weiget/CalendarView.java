package com.shkj.cm.calendarview.weiget;
...
import com.shkj.cm.calendarview.bean.AttrsBean;
import com.shkj.cm.calendarview.bean.DateBean;
import com.shkj.cm.calendarview.listener.CalendarViewAdapter;
import com.shkj.cm.calendarview.listener.OnMultiChooseListener;
import com.shkj.cm.calendarview.listener.OnPagerChangeListener;
import com.shkj.cm.calendarview.listener.OnSingleChooseListener;
import com.shkj.cm.calendarview.utils.CalendarUtil;
import com.shkj.cm.calendarview.utils.SolarUtil;
...
public class CalendarView extends ViewPager {
    ...
    private int[] lastClickDateForCurrentDate = new int[2];//记录当前日期的ViewPager position以及当前的日期
    ...
    public void init() {
        ...
        currentDatePosition = CalendarUtil.dateToPosition(initDate[0], initDate[1], startDate[0], startDate[1]);
        ...
        if (mAttrsBean.getChooseType() == 0) {
            ...
            int[] curDate = mAttrsBean.getCurrentDate();
            if (curDate != null) {
                lastClickDateForCurrentDate[0] = CalendarUtil.dateToPosition(curDate[0], curDate[1], startDate[0], startDate[1]);
                lastClickDateForCurrentDate[1] = curDate[2];
            }
        }
...
//                    || mAttrsBean.isSwitchChoose();
                boolean flag = lastClickDate[0] == position;
                boolean flag0fCurrent = lastClickDateForCurrentDate[0] == position;
                monthView.refresh(lastClickDate[1], flag);
                monthView.refreshCurrentDate(lastClickDateForCurrentDate[1], flag0fCurrent);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
     ...
     * @param dots     点标记的集合
     */
    public void refreshDot(List<Integer> dots) {
        mAttrsBean.setArrayDot(dots);
        MonthView monthView = calendarPagerAdapter.getViews().get(currentPosition);
        monthView.refreshDots();
    }
     ...
     * @return
     */
    public CalendarView setCurrentDate(String date) {
        int[] singleDate = CalendarUtil.strToArray(date);
        if (!isIllegal(singleDate)) {
            singleDate = null;
        }
        //设置当前日期
        mAttrsBean.setCurrentDate(singleDate);
        return this;
    }
